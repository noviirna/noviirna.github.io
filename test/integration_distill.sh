...
# !TODO! change distill_page to page index where we implement layout distill on a blog post
...
if [ ! -f "${distill_page}" ]; then
  ...
  # change exit to 0 because currently the blog post on my gh page is empty
  # when the distill_page name above is changed to a post that use distill layout
  # you can change exit to 1 to validate the unit test result
  exit 0
fi
